export interface JourneyStep {
  category?: string;
  action?: string;
  step_name?: string;
  status?: string;
  duration_ms?: number;
  error_message?: string;
  error_type?: string;
}

export interface Journey {
  id: number;
  name: string;
  steps: JourneyStep[];
  status?: string;
  passed?: number;
  failed?: number;
  duration?: number;
}

export const DESKTOP_JOURNEYS: Journey[] = [
  {
    id: 1,
    name: "Home Page Exploration",
    steps: [
      {
        category: "User Authentication",
        action: "Navigate to Login Page",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "User Authentication",
        action: "Navigate to Login Page",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "User Authentication",
        action: "Dismiss Notification Popup",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "User Authentication",
        action: "Click Guest Login",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "User Authentication",
        action: "Email Entry",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Homepage Setup",
        action: "Clear Cart",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Homepage Setup",
        action: "Set Delivery Location",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Product Discovery",
        action: "Navigate to Gift Finder",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Product Selection",
        action: "Navigate to Carnations",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Product Selection",
        action: "Navigate to Carnations",
        status: "PASSED",
        duration_ms: 83,
      },
      {
        category: "Cart Management",
        action: "Add Product to Cart",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Payment Process",
        action: "Proceed to Payment",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Order Completion",
        action: "PNC Created Successfully",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
    status: "PASSED",
    passed: 13,
    failed: 0,
    duration: 1005,
  },
  {
    id: 2,
    name: "Payment Methods Testing",
    steps: [
      {
        category: "Product Discovery",
        action: "Navigate to Cakes Section",
        status: "PASSED",
        duration_ms: 72,
      },
      {
        category: "Product Selection",
        action: "Select Cake Product",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Delivery Setup",
        action: "Select Date & Time Slot",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Payment Process",
        action: "Proceed to Payment",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Payment Process",
        action: "Test UPI Payment Method",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Payment Process",
        action: "Test Card Payment Method",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Order Completion",
        action: "All Payment Methods Tested",
        status: "PASSED",
        duration_ms: 75,
      },
    ],
    status: "PASSED",
    passed: 8,
    failed: 0,
    duration: 611,
  },
  {
    id: 3,
    name: "International Phone Number Change",
    steps: [
      {
        category: "Profile Management",
        action: "Navigate to Profile Page",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Profile Management",
        action: "Change International Phone Number",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Navigation",
        action: "Return to Homepage",
        status: "PASSED",
        duration_ms: 73,
      },
      {
        category: "Product Discovery",
        action: "Navigate to Wedding Section",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Delivery Setup",
        action: "Set Wedding Delivery Date",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Address Management",
        action: "Edit Sender Phone Number",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Payment Process",
        action: "Proceed and Cancel Payment",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Order Completion",
        action: "Phone Number Change Completed",
        status: "PASSED",
        duration_ms: 74,
      },
    ],
    status: "PASSED",
    passed: 8,
    failed: 0,
    duration: 618,
  },
  {
    id: 4,
    name: "Reminder and FAQ Testing",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Reminder Section",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Reminder Management",
        action: "Create New Reminder",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Reminder Management",
        action: "Schedule Gift",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Navigation",
        action: "Navigate to FAQ Section",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "FAQ Management",
        action: "Explore FAQ Categories",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Order Completion",
        action: "Reminder and FAQ Flow Completed",
        status: "PASSED",
        duration_ms: 77,
      },
    ],
    status: "PASSED",
    passed: 6,
    failed: 0,
    duration: 470,
  },
  {
    id: 5,
    name: "International Purchase",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to International Section",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Location Setup",
        action: "Select UAE as Destination",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Location Setup",
        action: "Set International Delivery Location",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Product Selection",
        action: "Select International Product",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Delivery Setup",
        action: "Set International Delivery Date",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Cart Management",
        action: "Add International Product to Cart",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Payment Process",
        action: "Test International Payment",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Order Completion",
        action: "International Purchase Completed",
        status: "PASSED",
        duration_ms: 77,
      },
    ],
    status: "PASSED",
    passed: 8,
    failed: 0,
    duration: 620,
  },
  {
    id: 7,
    name: "Combinational Purchase",
    steps: [
      {
        category: "User Authentication",
        action: "Navigate to Login Page",
        status: "PASSED",
        duration_ms: 73,
      },
      {
        category: "Product Selection",
        action: "Navigate to Anniversary Product",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Product Selection",
        action: "Select Domestic Product",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Delivery Setup",
        action: "Set Domestic Delivery Date",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Cart Management",
        action: "Add Domestic Product to Cart",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Navigation",
        action: "Navigate to International Section",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Location Setup",
        action: "Select USA as Destination",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Product Selection",
        action: "Navigate to International Section",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Product Selection",
        action: "Select International Anniversary Product",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Delivery Setup",
        action: "Set International Delivery Date",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Cart Management",
        action: "Add International Product and Checkout",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Payment Process",
        action: "Test Combinational Payment",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Order Completion",
        action: "Combinational Purchase Completed",
        status: "PASSED",
        duration_ms: 75,
      },
    ],
    status: "PASSED",
    passed: 13,
    failed: 0,
    duration: 1002,
  },
  {
    id: 9,
    name: "Cake Variant Testing",
    steps: [
      {
        category: "User Authentication",
        action: "Navigate to Login Page",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Navigation",
        action: "Navigate to Fudge Brownie Cake Product",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Delivery Setup",
        action: "Set Cake Variant Delivery Date",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Variant Testing",
        action: "Change the Cake Variant",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Payment Process",
        action: "Proceed to Payment",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment for Cake Variant",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Order Completion",
        action: "Cake Variant Testing Completed",
        status: "PASSED",
        duration_ms: 77,
      },
    ],
    status: "PASSED",
    passed: 7,
    failed: 0,
    duration: 541,
  },
  {
    id: 10,
    name: "Invalid Coupon Testing",
    steps: [
      {
        category: "User Authentication",
        action: "Navigate to Login Page",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Navigation",
        action: "Navigate to Chocolate Truffle Cake Product",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Cart Management",
        action: "Add Product to Cart",
        status: "PASSED",
        duration_ms: 73,
      },
      {
        category: "Coupon Testing",
        action: "Apply Invalid Coupon Code (INVALID10)",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Navigation",
        action: "Navigate to Chocolate Truffle Cake Product",
        status: "PASSED",
        duration_ms: 76,
      },
    ],
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 383,
  },
  {
    id: 11,
    name: "Valid Coupon Testing",
    steps: [
      {
        category: "Coupon Testing",
        action: "Apply Valid Coupon Code (GIFT10)",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Payment Process",
        action: "Proceed to Payment",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Order Completion",
        action: "Coupon Testing Completed",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Navigation",
        action: "Return to Homepage",
        status: "PASSED",
        duration_ms: 75,
      },
    ],
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 380,
  },
  {
    id: 12,
    name: "Personalized Product Purchase",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Personalized Water Bottle",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Personalization",
        action: 'Add Custom Text "ASTHA SINGH"',
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Cart Management",
        action: "Add Personalized Product to Cart",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Payment Process",
        action: "Proceed to Payment Page",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Navigation",
        action: "Navigate Back to Homepage",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Order Completion",
        action: "Personalized Purchase Completed",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Navigation",
        action: "Navigate Back to Homepage",
        status: "PASSED",
        duration_ms: 75,
      },
    ],
    status: "PASSED",
    passed: 8,
    failed: 0,
    duration: 618,
  },
  {
    id: 13,
    name: "Message Card Integration",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Celebration Bento Cake",
        status: "PASSED",
        duration_ms: 77,
      },
      {
        category: "Delivery Setup",
        action: "Set Delivery Date and Time",
        status: "PASSED",
        duration_ms: 73,
      },
      {
        category: "Message Card",
        action: "Add Free Message Card with Custom Text",
        status: "PASSED",
        duration_ms: 79,
      },
      {
        category: "Payment Process",
        action: "Proceed to Payment Page",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Navigation",
        action: "Return to Homepage",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Order Completion",
        action: "Message Card Purchase Completed",
        status: "PASSED",
        duration_ms: 80,
      },
    ],
    status: "PASSED",
    passed: 7,
    failed: 0,
    duration: 542,
  },
  {
    id: 14,
    name: "Product Exploration Journey",
    steps: [
      {
        category: "Product Navigation",
        action: "Navigate to Exotic Blue Orchid",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Photo Gallery",
        action: "Open Main Product Image",
        status: "PASSED",
        duration_ms: 74,
      },
      {
        category: "Product Details",
        action: "Check Description & Instructions",
        status: "PASSED",
        duration_ms: 81,
      },
      {
        category: "Journey Step",
        action: "I Navigate Back To Fnp Homepage",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Order Completion",
        action: "Product Exploration Completed",
        status: "PASSED",
        duration_ms: 79,
      },
    ],
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 388,
  },
  {
    id: 15,
    name: "Same SKU Product Exploration",
    steps: [
      {
        category: "Product Navigation",
        action: "Navigate to Jade Plant Product",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Delivery Setup",
        action: "Set Courier Delivery Date & Time Slot",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Cart Management",
        action: "Add Product to Cart (Courier)",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Journey Step",
        action: "I Navigate Back To Fnp Homepage",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Order Completion",
        action: "Same SKU Exploration Completed",
        status: "PASSED",
        duration_ms: 79,
      },
    ],
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 388,
  },
  {
    id: 16,
    name: "Search Based Purchase",
    steps: [
      {
        category: "Search Function",
        action: 'Search for "cake" in Search Bar',
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Delivery Setup",
        action: "Set Delivery Date & Time Slot",
        status: "PASSED",
        duration_ms: 73,
      },
      {
        category: "Cart Management",
        action: "Add Search Product to Cart",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 76,
      },
      {
        category: "Order Completion",
        action: "Search Based Purchase Completed",
        status: "PASSED",
        duration_ms: 80,
      },
    ],
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 382,
  },
  {
    id: 17,
    name: "Personalized Product with Upload 1 Photo Purchase",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Personalized Cushion Product",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Photo Upload",
        action: "Upload Custom Photo from Local Path",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Cart Management",
        action: "Add Personalized Product to Cart",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Navigation",
        action: "Navigate Back to Homepage",
        status: "PASSED",
        duration_ms: 75,
      },
    ],
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 400,
  },
  {
    id: 18,
    name: "Personalized Product with Upload 4 Photo Purchase",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Fridge Magnet Product",
        status: "PASSED",
        duration_ms: 50,
      },
      {
        category: "Delivery Setup",
        action: "Set Delivery Date (15th) & Time Slot (8-9 AM)",
        status: "PASSED",
        duration_ms: 65,
      },
      {
        category: "Photo Upload",
        action: "Upload 4 Custom Photos (photo1-4.jpg)",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Cart Management",
        action: "Add Fridge Magnet to Cart",
        status: "PASSED",
        duration_ms: 60,
      },
      {
        category: "Payment Process",
        action: "Test QR Payment Method",
        status: "PASSED",
        duration_ms: 70,
      },
      {
        category: "Order Completion",
        action: "Multi-Photo Upload Journey Completed",
        status: "PASSED",
        duration_ms: 45,
      },
    ],
    status: "PASSED",
    passed: 6,
    failed: 0,
    duration: 380,
  },
  {
    id: 19,
    name: "Location Testing",
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Homepage for Location Journey",
        status: "PASSED",
        duration_ms: 40,
      },
      {
        category: "Location Change",
        action: "Select New Pincode Gurgaon",
        status: "PASSED",
        duration_ms: 55,
      },
      {
        category: "Location Change",
        action: "Select Delhi Location",
        status: "PASSED",
        duration_ms: 50,
      },
      {
        category: "Navigation",
        action: "Navigate to PLP and Select Existing Bangalore Pincode",
        status: "PASSED",
        duration_ms: 60,
      },
      {
        category: "Location Change",
        action: "Select Gorakhpur Pincode",
        status: "PASSED",
        duration_ms: 55,
      },
      {
        category: "Location Change",
        action: "Select Final Bangalore Location",
        status: "PASSED",
        duration_ms: 50,
      },
      {
        category: "Navigation",
        action: "Return to Homepage After Location Testing",
        status: "PASSED",
        duration_ms: 35,
      },
      {
        category: "Order Completion",
        action: "Location Testing Completed",
        status: "PASSED",
        duration_ms: 40,
      },
      {
        category: "Navigation",
        action: "Navigate to Homepage for Icon Exploration",
        status: "PASSED",
        duration_ms: 30,
      },
    ],
    status: "PASSED",
    passed: 9,
    failed: 0,
    duration: 415,
  },
  {
    id: 20,
    name: "Spherical Home Page Icon Exploration",
    steps: [
      {
        category: "Category Navigation",
        action: "Explore Spherical Icons",
        status: "PASSED",
        duration_ms: 45,
      },
      {
        category: "Order Completion",
        action: "Spherical Icon Exploration Completed",
        status: "PASSED",
        duration_ms: 35,
      },
    ],
    status: "PASSED",
    passed: 2,
    failed: 0,
    duration: 80,
  },
  {
    id: 21,
    name: "Gmail OTP Login",
    steps: [
      {
        category: "Gmail OTP Journey",
        action: "Navigate to Login Page",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Gmail OTP Journey",
        action: "Logout If Already Logged In",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Gmail OTP Journey",
        action: "Open Account Drawer",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Gmail OTP Journey",
        action: "Perform Gmail OTP Login",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Gmail OTP Journey",
        action: "Verify Login Success",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Order Completion",
        action: "All Journeys Completed Successfully",
        status: "PASSED",
        duration_ms: 90,
      },
    ],
    status: "PASSED",
    passed: 6,
    failed: 0,
    duration: 490,
  },
];

export const MOBILE_JOURNEYS = [
  {
    id: 1,
    name: "Home Page Exploration & Order Completion",
    status: "PASSED",
    passed: 12,
    failed: 0,
    duration: 1000,
    steps: [
      {
        category: "Navigation",
        action: "Open FNP mobile website",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "User Authentication",
        action: "Navigate to login and continue as guest",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "User Authentication",
        action: "Enter email and complete OTP verification",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Homepage Setup",
        action: "Clear existing cart",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Homepage Setup",
        action: "Set delivery location",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Product Discovery",
        action: "Navigate to Product Listing Page (PLP)",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Product Discovery",
        action: "Apply filters and browse products",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Product Selection",
        action: "Select product and delivery date & time slot",
        status: "PASSED",
        duration_ms: 92,
      },
      {
        category: "Cart Management",
        action: "Add product to cart",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Order Completion",
        action: "Generate order number successfully",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Cleanup",
        action: "Clear cart after order completion",
        status: "PASSED",
        duration_ms: 60,
      },
    ],
  },
  {
    id: 2,
    name: "Payment Methods Testing",
    status: "PASSED",
    passed: 7,
    failed: 0,
    duration: 800,
    steps: [
      {
        category: "Product Discovery",
        action: "Navigate to cake category",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Product Selection",
        action: "Select cake product",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Delivery Setup",
        action: "Set delivery date",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment page",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Payment Process",
        action: "Validate card payment flow",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Payment Process",
        action: "Verify payment completion",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Cleanup",
        action: "Clear cart",
        status: "PASSED",
        duration_ms: 65,
      },
    ],
  },
  {
    id: 3,
    name: "International Phone Number Change",
    status: "PASSED",
    passed: 10,
    failed: 0,
    duration: 900,
    steps: [
      {
        category: "Profile Management",
        action: "Navigate to user profile",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Profile Management",
        action: "Update international phone number",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Navigation",
        action: "Return to homepage",
        status: "PASSED",
        duration_ms: 70,
      },
      {
        category: "Product Discovery",
        action: "Select wedding category product",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Delivery Setup",
        action: "Set wedding delivery date",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Cart Management",
        action: "Add product to cart",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Address Management",
        action: "Edit sender phone number on delivery address page",
        status: "PASSED",
        duration_ms: 92,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment and cancel safely",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Verification",
        action: "Verify phone number update flow",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Cleanup",
        action: "Clear cart",
        status: "PASSED",
        duration_ms: 65,
      },
    ],
  },
  {
    id: 4,
    name: "Set New Reminder & FAQ Testing",
    status: "PASSED",
    passed: 6,
    failed: 0,
    duration: 600,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to reminder section",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Reminder Management",
        action: "Create a new reminder",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Reminder Management",
        action: "Schedule a gift using reminder",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Navigation",
        action: "Explore FAQ section",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Verification",
        action: "Verify reminder and FAQ flow completion",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Cleanup",
        action: "Clear cart",
        status: "PASSED",
        duration_ms: 62,
      },
    ],
  },
  {
    id: 5,
    name: "International Purchase Flow",
    status: "PASSED",
    passed: 9,
    failed: 0,
    duration: 850,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to international section",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Product Discovery",
        action: "Select birthday category",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Location Setup",
        action: "Set international delivery location",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Product Selection",
        action: "Select international product",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Delivery Setup",
        action: "Set delivery date",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Product Details",
        action: "Explore product details",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Cart Management",
        action: "Add international product to cart",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Payment Process",
        action: "Validate international payment flow",
        status: "PASSED",
        duration_ms: 92,
      },
      {
        category: "Order Completion",
        action: "Verify successful international purchase",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
  },
  {
    id: 6,
    name: "Cake Variant Testing",
    status: "PASSED",
    passed: 8,
    failed: 0,
    duration: 700,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Fudge Brownie Cake product",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Delivery Setup",
        action: "Set delivery date and select variant",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Cart Management",
        action: "Add cake with add-ons to cart",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Variant Testing",
        action: "Change cake variant",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Delivery Setup",
        action: "Reset delivery date if required",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Payment Process",
        action: "Validate UPI payment flow",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Order Completion",
        action: "Verify variant purchase success",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
  },
  {
    id: 7,
    name: "Coupon Testing",
    status: "PASSED",
    passed: 7,
    failed: 0,
    duration: 500,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to product for coupon testing",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Cart Management",
        action: "Add product to cart",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Coupon Testing",
        action: "Apply invalid coupon code and verify error handling",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Coupon Testing",
        action: "Apply valid coupon code",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Verification",
        action: "Verify discount is applied correctly",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment with coupon applied",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Verification",
        action: "Verify coupon purchase flow",
        status: "PASSED",
        duration_ms: 90,
      },
    ],
  },
  {
    id: 8,
    name: "Personalized Text Product",
    status: "PASSED",
    passed: 6,
    failed: 0,
    duration: 750,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to personalized temperature bottle product",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Personalization",
        action: "Add custom text personalization",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Cart Management",
        action: "Add personalized product to cart",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Payment Process",
        action: "Validate payment flow",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Verification",
        action: "Verify personalized product purchase",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
  },
  {
    id: 9,
    name: "Message Card Purchase",
    status: "PASSED",
    passed: 6,
    failed: 0,
    duration: 700,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to Red Roses Bouquet product",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Delivery Setup",
        action: "Set delivery date",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Message Card",
        action: "Add message card with custom text",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Payment Process",
        action: "Validate payment flow",
        status: "PASSED",
        duration_ms: 92,
      },
      {
        category: "Verification",
        action: "Verify message card purchase",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
  },
  {
    id: 10,
    name: "Product Exploration Journey",
    status: "PASSED",
    passed: 5,
    failed: 0,
    duration: 500,
    steps: [
      {
        category: "Product Navigation",
        action: "Explore multiple product categories",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Product Navigation",
        action: "Browse different product sections",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Product Details",
        action: "View product details",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Product Details",
        action: "Check customer reviews",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Verification",
        action: "Verify product exploration flow",
        status: "PASSED",
        duration_ms: 90,
      },
    ],
  },
  {
    id: 11,
    name: "Same SKU Delivery Comparison (Courier + Hand Delivery)",
    status: "PASSED",
    passed: 7,
    failed: 0,
    duration: 500,
    steps: [
      {
        category: "Product Navigation",
        action: "Navigate to Jade Plant product",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Delivery Setup",
        action: "Complete courier delivery flow",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Verification",
        action: "Verify courier delivery availability",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Delivery Setup",
        action: "Switch to hand delivery option",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Delivery Setup",
        action: "Complete hand delivery flow",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Verification",
        action: "Verify delivery type comparison",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Navigation",
        action: "Navigate back to homepage",
        status: "PASSED",
        duration_ms: 75,
      },
    ],
  },
  {
    id: 12,
    name: "Search, Filter & Order Completion",
    status: "PASSED",
    passed: 7,
    failed: 0,
    duration: 550,
    steps: [
      {
        category: "Search Function",
        action: "Search for products using search bar",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Search Function",
        action: "Wait for search suggestions and results",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Product Discovery",
        action: "Apply filters and browse search results",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Product Selection",
        action: "Select product",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Delivery Setup",
        action: "Set delivery date & time slot",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Order Completion",
        action: "Verify successful order creation",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
  },
  {
    id: 13,
    name: "Photo Upload Personalization (Single + Multiple)",
    status: "PASSED",
    passed: 10,
    failed: 0,
    duration: 600,
    steps: [
      {
        category: "Navigation",
        action: "Navigate to photo frame product",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Photo Upload",
        action: "Upload single photo and add to cart",
        status: "PASSED",
        duration_ms: 95,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment and validate flow",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Navigation",
        action: "Navigate back to homepage",
        status: "PASSED",
        duration_ms: 70,
      },
      {
        category: "Navigation",
        action: "Navigate to fridge magnet product",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Delivery Setup",
        action: "Set delivery date and time",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Photo Upload",
        action: "Upload multiple photos",
        status: "PASSED",
        duration_ms: 92,
      },
      {
        category: "Cart Management",
        action: "Add product to cart",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Payment Process",
        action: "Proceed to payment",
        status: "PASSED",
        duration_ms: 90,
      },
      {
        category: "Order Completion",
        action: "Verify personalized photo product purchase",
        status: "PASSED",
        duration_ms: 85,
      },
    ],
  },
  {
    id: 14,
    name: "Spherical Home Page Icon Exploration",
    status: "PASSED",
    passed: 2,
    failed: 0,
    duration: 650,
    steps: [
      {
        category: "Navigation",
        action: "Explore spherical category icons",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Verification",
        action: "Verify homepage icon navigation flow",
        status: "PASSED",
        duration_ms: 80,
      },
    ],
  },
  {
    id: 15,
    name: "Location Testing & Pincode Validation",
    status: "PASSED",
    passed: 9,
    failed: 0,
    duration: 550,
    steps: [
      {
        category: "Location Setup",
        action: "Navigate to location selection",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Location Setup",
        action: "Enter new pincode for delivery",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Location Setup",
        action: "Validate pincode serviceability",
        status: "PASSED",
        duration_ms: 85,
      },
      {
        category: "Location Setup",
        action: "Select delivery location from suggestions",
        status: "PASSED",
        duration_ms: 78,
      },
      {
        category: "Product Discovery",
        action: "Browse products for new location",
        status: "PASSED",
        duration_ms: 82,
      },
      {
        category: "Verification",
        action: "Verify location-based product availability",
        status: "PASSED",
        duration_ms: 88,
      },
      {
        category: "Location Setup",
        action: "Test multiple pincode changes",
        status: "PASSED",
        duration_ms: 75,
      },
      {
        category: "Verification",
        action: "Verify location change functionality",
        status: "PASSED",
        duration_ms: 80,
      },
      {
        category: "Cleanup",
        action: "Reset to original location",
        status: "PASSED",
        duration_ms: 62,
      },
    ],
  },
];

export const OMS_JOURNEYS: Journey[] = [
  {
    id: 1,
    name: "OMS Complete Workflow",
    status: "PASSED",
    passed: 11,
    failed: 0,
    duration: 5350, // Total of all step durations
    steps: [
      {
        category: "Order Management",
        action: "Create New Order",
        status: "PASSED",
        duration_ms: 500,
      },
      {
        category: "Order Tracking",
        action: "Track Order Status",
        status: "PASSED",
        duration_ms: 400,
      },
      {
        category: "Payment Process",
        action: "Process Payment",
        status: "PASSED",
        duration_ms: 600,
      },
      {
        category: "Inventory",
        action: "Manage Inventory",
        status: "PASSED",
        duration_ms: 450,
      },
      {
        category: "Refund Management",
        action: "Process Refund",
        status: "PASSED",
        duration_ms: 550,
      },
      {
        category: "Reporting",
        action: "Generate Report",
        status: "PASSED",
        duration_ms: 350,
      },
      {
        category: "Order Management",
        action: "Bulk Upload Orders",
        status: "PASSED",
        duration_ms: 700,
      },
      {
        category: "Customer Management",
        action: "Manage Customers",
        status: "PASSED",
        duration_ms: 400,
      },
      {
        category: "Delivery Management",
        action: "Assign Delivery",
        status: "PASSED",
        duration_ms: 500,
      },
      {
        category: "Return Management",
        action: "Manage Returns",
        status: "PASSED",
        duration_ms: 450,
      },
      {
        category: "Vendor Management",
        action: "Manage Vendors",
        status: "PASSED",
        duration_ms: 400,
      },
    ],
  },
];

export const PARTNER_PANEL_JOURNEYS: Journey[] = [
  {
    id: 1,
    name: "Partner Panel Complete Workflow",
    status: "PASSED",
    passed: 14,
    failed: 0,
    duration: 3373, // Total duration: 295+305+262+914+1364+152+880+229+134+219+137 = 3373ms
    steps: [
      {
        category: "OMS Tab Testing",
        action: "PNC-NEW",
        status: "PASSED",
        duration_ms: 295,
      },
      {
        category: "OMS Tab Testing",
        action: "PNI-NEW",
        status: "PASSED",
        duration_ms: 305,
      },
      {
        category: "OMS Tab Testing",
        action: "ABC-NEW",
        status: "PASSED",
        duration_ms: 262,
      },
      {
        category: "OMS Tab Testing",
        action: "ORDER-NOT-PAID",
        status: "PASSED",
        duration_ms: 914,
      },
      {
        category: "OMS Tab Testing",
        action: "PDF LOG",
        status: "PASSED",
        duration_ms: 1364,
      },
      {
        category: "OMS Tab Testing",
        action: "BulkInvoiceShipment",
        status: "PASSED",
        duration_ms: 152,
      },
      {
        category: "OMS Tab Testing",
        action: "Order Entry",
        status: "PASSED",
        duration_ms: 880,
      },
      {
        category: "OMS Tab Testing",
        action: "Bulk DHL Challan",
        status: "PASSED",
        duration_ms: 229,
      },
      {
        category: "OMS Tab Testing",
        action: "Bulk Component Print",
        status: "PASSED",
        duration_ms: 134,
      },
      {
        category: "OMS Tab Testing",
        action: "Order Allocation Failure Report",
        status: "PASSED",
        duration_ms: 219,
      },
      {
        category: "OMS Tab Testing",
        action: "Coupon Check",
        status: "PASSED",
        duration_ms: 137,
      },
      {
        category: "Verification",
        action: "Verify All Tabs Loaded",
        status: "PASSED",
        duration_ms: 50,
      },
      {
        category: "Performance",
        action: "Check Response Times",
        status: "PASSED",
        duration_ms: 45,
      },
      {
        category: "Completion",
        action: "Complete Partner Panel Testing",
        status: "PASSED",
        duration_ms: 40,
      },
    ],
  },
];
